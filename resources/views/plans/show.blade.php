...
@stop
@section('content')
    <x-alerts /><div class="card"><div class="card-body"><dl class="row mb-0"><dt class="col-sm-3">Nome</dt><dd class="col-sm-9">{{ $plan->name }}</dd><dt class="col-sm-3">Descrição</dt><dd class="col-sm-9">{{ $plan->description ?: '-' }}</dd><dt class="col-sm-3">Duração</dt><dd class="col-sm-9">{{ $plan->duration_months }} {{ $plan->duration_months === 1 ? 'mês' : 'meses' }}</dd><dt class="col-sm-3">Valor mensal</dt><dd class="col-sm-9">R$ {{ number_format((float) $plan->price, 2, ',', '.') }}</dd><dt class="col-sm-3">Status</dt><dd class="col-sm-9"><span class="badge text-bg-{{ $plan->active ? 'success' : 'secondary' }}">{{ $plan->active ? 'Ativo' : 'Inativo' }}</span></dd></dl></div></div>
@stop
