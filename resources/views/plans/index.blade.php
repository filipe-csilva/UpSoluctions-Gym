...
@section('content')
    ...
    <div class="card"><div class="card-body table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Nome</th><th>Duração</th><th>Valor</th><th>Status</th><th class="text-end">Ações</th></tr></thead><tbody>
        @forelse($plans as $plan)<tr><td>{{ $plan->name }}</td><td>{{ $plan->duration_months }} {{ $plan->duration_months === 1 ? 'mês' : 'meses' }}</td><td>R$ {{ number_format((float) $plan->price, 2, ',', '.') }}</td><td><span class="status-pill status-pill-{{ $plan->active ? 'success' : 'neutral' }}">{{ $plan->active ? 'Ativo' : 'Inativo' }}</span></td><td class="text-end"><a href="{{ route('plans.show', $plan) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i> Visualizar</a></td></tr>@empty<tr><td colspan="5" class="text-center text-muted">Nenhum plano cadastrado.</td></tr>@endforelse
