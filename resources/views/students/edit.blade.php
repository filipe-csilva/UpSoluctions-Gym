...
@section('content')
    ...
    <form method="POST" action="{{ route('students.update', $student) }}" data-viacep>
        ...
        <div class="card"><div class="card-body"><div class="row">
            ...
            <div class="form-group col-md-6"><label for="active">Status</label><select id="active" name="active" class="form-control"><option value="1" @selected(old('active', $student->user->active))>Ativo</option><option value="0" @selected(! old('active', $student->user->active))>Inativo</option></select></div>
            <div class="form-group col-md-6"><label for="role">Perfil de acesso</label><select id="role" name="role" class="form-control" required>@foreach ($roles as $role)<option value="{{ $role->value }}" @selected(old('role', $student->user->role?->value) === $role->value)>{{ match ($role) { \App\Enums\UserRole::ADMIN => 'Administrador', \App\Enums\UserRole::MANAGER => 'Gerente', \App\Enums\UserRole::TEACHER => 'Professor', \App\Enums\UserRole::STUDENT => 'Aluno', \App\Enums\UserRole::FINANCIAL => 'Financeiro' } }}</option>@endforeach</select></div>
