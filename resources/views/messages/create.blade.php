    ...
    <div class="card"><div class="card-body">
        <form method="POST" action="{{ route('messages.store') }}">
            ...
            @if (in_array($role, ['admin', 'manager'], true))
                <div class="mb-3"><label class="form-label" for="audience">Enviar para</label><select id="audience" name="audience" class="form-select" required><option value="all">Todos</option><option value="unit">Uma unidade</option></select></div>
                <div class="mb-3"><label class="form-label" for="unit_id">Unidade</label><select id="unit_id" name="unit_id" class="form-select"><option value="">Selecione a unidade</option>@foreach($units as $unit)<option value="{{ $unit->id }}">{{ $unit->name }}</option>@endforeach</select></div>
            @else
                @if ($role === 'student')
                    <div class="mb-3"><label class="form-label" for="audience">Enviar para</label><select id="audience" name="audience" class="form-select" required><option value="direct">Meu instrutor</option><option value="unit">Minha unidade</option></select></div>
